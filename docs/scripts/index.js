...
function fetchObjktsCanned() {

  fetch('canned-data.json')
    .then(response => response.json())
    .then(data => {
      // console.log(data)

      objkts = data.data.generativeToken.entireCollection
      // objkts = _.uniqBy(objkts, 'generationHash') // remove duplicate geHash pieces - OG fxBug
      // objkts = _.filter(objkts, objkt => {
      //   return objkt.name.split(" ")[0] == "Nouveau" && objkt.metadata.name.split(" ")[0] == "Nouveau"  // remove pieces by others - OG fxBug
      // })
      objkts = _.sortBy(objkts, 'iteration')
      origObjkts = _.clone(objkts)
      // console.log("origObjkts: ", origObjkts)
      // setup()
      fetchListings()
...
function fetchListings() {
  ...
  let queryString = `
    ...
    .then(r => r.json())
    .then(data => {
      console.log("activeListings Data: ", data)
      let listingsObjkts = data.data.generativeToken.entireCollection
      // console.log('listingsObjkts: ', listingsObjkts)
      _.each(listingsObjkts, listingObjkt => {
        let ob = _.find(objkts, objkt => { 

          // console.log("origObjkts: ", listingObjkt.listings)
          return objkt.id == listingObjkt.id
        })
        if (ob) ob.activeListing = listingObjkt.activeListing
      })
      kickoffMany()
    })
...
function fetchObjkts() {
  ...
  function doFetch() {
    ...
    .then(data => {
      ...
      // objkts = _.uniqBy(objkts, 'generationHash') // remove duplicate geHash pieces - OG fxBug
      // objkts = _.filter(objkts, objkt => {
      //   return objkt.name.split(" ")[0] == "Nouveau" // remove pieces by others - OG fxBug
      // })
      // objkts = _.sortBy(objkts, 'iteration')
      // origObjkts = _.clone(objkts)
      // // console.log("origObjkts: ", origObjkts)
...
function sortByPrice() {
  ...
  objkts = _.orderBy(objkts, [objkt => {
    ...
    if (objkt.activeListing) {
      rank = objkt.activeListing.price
    }
    return rank
  }],['desc'])
...
function updateReadout() {
  ...
  if (data.activeListing) {
    ...
    offerLink.innerHTML = "🟢 " + String(data.activeListing.price / 1000000) + "tz"
